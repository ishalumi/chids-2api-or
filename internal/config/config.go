...
type Config struct {
	...
	AdminPass     string
	AdminPath     string
	RegisterProxy string // 注册代理 URL (http://user:pass@host:port 或 socks5://host:port)
	ChatProxy     string // 对话代理 URL
}
...
func Load() *Config {
	return &Config{
		...
		RegisterProxy: getEnv("REGISTER_PROXY", ""), // 格式: http://user:pass@host:port 或 socks5://host:port
		ChatProxy:     getEnv("CHAT_PROXY", ""),     // 格式: http://user:pass@host:port 或 socks5://host:port
	}
}
